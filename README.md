...
| Layer | Technology |
|---|---|
| WhatsApp | [Baileys](https://github.com/WhiskeySockets/Baileys) (WhatsApp protocol over WebSocket, no browser) |
...
### Prerequisites
- Node.js 20+ (no browser needed: the bot speaks the protocol directly)
...
> **macOS:** edit the plist generated by `pm2 startup`, swapping `/bin/sh -c` for `/bin/zsh -l -c`. Without a login shell the process inherits a minimal environment and cannot find Node.

### 5. Or run it on a server (Docker)

To keep the bot running even with your own machine off, there is a ready-made
image for Linux VPS (ARM or x86), under 300 MB:

```bash
docker compose -f deploy/docker-compose.yml up -d --build
```

Full walkthrough (free Oracle Cloud VM, remote QR scan, audio transcription on
the server) in [deploy/README.md](deploy/README.md) — written in Portuguese.
...
3. **The browser was the bottleneck** → while the bot ran on `whatsapp-web.js`, headless Chromium held 1.3 GB of RAM around the clock, left orphaned processes locking the profile after every abrupt restart, and broke on each WhatsApp Web change. Moving to Baileys, which speaks the protocol over WebSocket, dropped usage below 60 MB and removed that whole class of problem.
4. **Reconnecting takes care** → unlike a browser-backed library, a dropped connection no longer kills the process: the "connected" event fires again on every reconnect. Registering schedules and handlers inside it duplicates everything (21 times in a 3-minute test here). Register once per process, and when recreating the socket, close the previous one and drop its listeners, or connections multiply until WhatsApp kills them all with `conflict`.
...
- [x] VPS deploy guide (Docker + Oracle Cloud ARM) — see `deploy/README.md`
